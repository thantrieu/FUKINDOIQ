...
fun String.sortByLengthDESC(): String {
    // sắp xếp các từ theo thứ tự giảm dần độ dài từ sau đó đảo ngược
    val words = this.split(" ").sortedBy { w -> w.length }.reversed()
...
fun main() {
    ...
    println("Các từ trong câu sau khi sắp xếp: ${str1.sortByLengthDESC()}")
    println("Các từ trong câu sau khi sắp xếp: ${str2.sortByLengthDESC()}")
}